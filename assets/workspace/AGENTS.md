...
14. review whether the page's UX supports its primary user task and CTA
15. record `UX review notes` and `UX review result` in the page spec, using `pass`, `needs-rework`, or `fail` for the result
...
- `UX review notes` and `UX review result` are filled in the page spec
